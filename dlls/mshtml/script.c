...
static void install_intl_shim(ScriptHost *script_host)
{
    static const WCHAR intl_shim[] =
        ...
        L"this._locale = locale || 'en-US'; this._options = opts(options); "
        L"var self = this; this.format = function(n){ return formatNumber(self, n); }; "
        L"} "
        L"function formatNumber(formatter, n){ "
        L"var num = Number(n); "
        L"var min = formatter._options.minimumFractionDigits; "
        L"var max = formatter._options.maximumFractionDigits; "
        L"if(typeof max === 'number'){ "
        L"var text = num.toFixed(max), point = text.indexOf('.'); "
        L"var keep = (typeof min === 'number') ? min : 0; "
        L"while(point >= 0 && text.charAt(text.length - 1) === '0' && text.length - point - 1 > keep) "
        L"text = text.slice(0, -1); "
        L"if(text.charAt(text.length - 1) === '.') text = text.slice(0, -1); "
        L"return text; "
        L"} "
