...
struct renderer_root
{
    ...
    BOOL present_commit_proven;
    BOOL present_wsi_unavailable;
...
{
    if (!root->input_authorized) return;
    switch (event->type)
    {
    case WINEWAYLAND_HOST_INPUT_FOCUS:
        root->keyboard_focus_pending = TRUE;
        break;
    case WINEWAYLAND_HOST_INPUT_KEY:
        root->keyboard_snapshot_pending = TRUE;
        break;
    case WINEWAYLAND_HOST_INPUT_MODIFIERS:
        root->keyboard_modifiers_pending = root->keyboard_modifiers_valid;
        break;
    case WINEWAYLAND_HOST_INPUT_POINTER_BUTTON:
        root->pointer_reset_pending = TRUE;
        break;
    case WINEWAYLAND_HOST_INPUT_RESET:
        if (event->flags & WINEWAYLAND_HOST_INPUT_RESET_KEYS)
            root->keyboard_snapshot_pending = TRUE;
        if (event->flags & WINEWAYLAND_HOST_INPUT_RESET_BUTTONS)
            root->pointer_reset_pending = TRUE;
        break;
    }
}
        ...
        struct winewayland_host_input_event *event, BOOL coalesce)
{
    ...
    if (renderer.input_count == ARRAY_SIZE(renderer.input_events))
    {
        ...
        if (renderer.input_count == ARRAY_SIZE(renderer.input_events))
        {
            tail = &renderer.input_events[renderer.input_head];
            /* Losing an edge affects its origin, which may be a different
             * root from this new event. Reconcile current state, not the
             * evicted event's stale payload. */
            for (index = 0; index < ARRAY_SIZE(renderer.roots); ++index)
                if (renderer.roots[index].root_identity == tail->root_identity &&
                    renderer.roots[index].root_generation == tail->root_generation)
                    mark_renderer_input_lost(&renderer.roots[index], tail);
...
static void queue_renderer_keyboard_focus(struct renderer_root *root)
{
    struct winewayland_host_input_event event = {0};

    if (!root || !root->input_authorized || !root->keyboard_focus_pending ||
        renderer.input_count == ARRAY_SIZE(renderer.input_events)) return;
    event.type = WINEWAYLAND_HOST_INPUT_FOCUS;
    event.state = root->keyboard_focused;
    queue_renderer_input(root, &event, FALSE);
    root->keyboard_focus_pending = FALSE;
}
...
static void queue_renderer_keyboard_snapshot(struct renderer_root *root)
{
    ...
    if (!root || !root->input_authorized || !root->keyboard_snapshot_pending ||
        root->keyboard_focus_pending) return;
    count = root->keyboard_key_count + 1 + root->keyboard_modifiers_valid;
    /* Wait for room for the complete snapshot. Repeated partial resets for
     * several roots could otherwise fill every newly freed queue slot. */
...
static void set_renderer_input_authorized(struct renderer_root *root, BOOL authorized)
{
    ...
    queue_renderer_keyboard_focus(root);
    queue_renderer_keyboard_snapshot(root);
    queue_renderer_keyboard_modifiers(root);
}

static void queue_pending_renderer_input(void)
{
    unsigned int i;

    for (i = 0; i < ARRAY_SIZE(renderer.roots); ++i)
    {
        struct renderer_root *root = &renderer.roots[i];

        if (root->input_authorized && root->pointer_reset_pending &&
            renderer.input_count < ARRAY_SIZE(renderer.input_events))
        {
...
static void leave_renderer_pointer_root(void)
{
    /* The compositor can take an interactive grab and consume the release.
     * Cancel only this root's buttons; keyboard focus is independent. */
        ...
        uint32_t time, uint32_t key, uint32_t state)
{
    ...
    if (!root->input_authorized || root->keyboard_focus_pending ||
...
static NTSTATUS renderer_keyboard_enter_self_test(void)
{
    ...
    else
    {
        ...
        if (store_renderer_keyboard_enter(pending_root, 29, &keys) ||
            renderer.input_count != ARRAY_SIZE(renderer.input_events) - 3 ||
        ...
        renderer.input_head = renderer.input_count = 0;
        pending_root->keyboard_focused = pending_root->keyboard_focus_pending = TRUE;
        queue_renderer_keyboard_focus(pending_root);
        for (i = 1; i < ARRAY_SIZE(renderer.input_events); ++i)
            queue_renderer_input(&other_root, &queued, FALSE);
        queue_renderer_input(&other_root, &queued, FALSE);
        if (!renderer.input_overflow || !pending_root->keyboard_focus_pending)
            status = STATUS_DATA_ERROR;
        pending_root->keyboard_focused = FALSE;
        if (get_renderer_input(&dequeued) || pending_root->keyboard_focus_pending ||
            renderer.input_events[(renderer.input_head + renderer.input_count - 1) %
                    ARRAY_SIZE(renderer.input_events)].type != WINEWAYLAND_HOST_INPUT_FOCUS ||
            renderer.input_events[(renderer.input_head + renderer.input_count - 1) %
                    ARRAY_SIZE(renderer.input_events)].state)
            status = STATUS_DATA_ERROR;
        ...
        for (i = 1; i < ARRAY_SIZE(renderer.input_events); ++i)
            queue_renderer_input(&other_root, &queued, FALSE);
        queue_renderer_input(&other_root, &queued, FALSE);
        if (!pending_root->pointer_reset_pending || get_renderer_input(&dequeued) ||
            pending_root->pointer_reset_pending)
            status = STATUS_DATA_ERROR;
        event = &renderer.input_events[(renderer.input_head + renderer.input_count - 1) %
                ARRAY_SIZE(renderer.input_events)];
        if (event->type != WINEWAYLAND_HOST_INPUT_RESET ||
            event->flags != WINEWAYLAND_HOST_INPUT_RESET_BUTTONS ||
            event->root_identity != pending_root->root_identity)
            status = STATUS_DATA_ERROR;

        /* Evicting A's release must reconcile A, not cancel B's new press. */
        renderer.input_head = renderer.input_count = 0;
        pending_root->keyboard_key_count = 0;
        queued.type = WINEWAYLAND_HOST_INPUT_KEY;
        queued.state = WL_KEYBOARD_KEY_STATE_RELEASED;
        queue_renderer_input(pending_root, &queued, FALSE);
        queued.state = WL_KEYBOARD_KEY_STATE_PRESSED;
        for (i = 1; i < ARRAY_SIZE(renderer.input_events); ++i)
            queue_renderer_input(&other_root, &queued, FALSE);
        queue_renderer_input(&other_root, &queued, FALSE);
        if (!pending_root->keyboard_snapshot_pending || get_renderer_input(&dequeued) ||
            pending_root->keyboard_snapshot_pending)
            status = STATUS_DATA_ERROR;
        event = &renderer.input_events[(renderer.input_head + renderer.input_count - 1) %
                ARRAY_SIZE(renderer.input_events)];
        ...
        /* Several pending snapshots must not refill each freed slot with
         * partial resets and prevent the complete snapshots from fitting. */
        for (i = 0; i < ARRAY_SIZE(renderer.roots); ++i)
            if (!renderer.roots[i].root_identity)
            {
                pending_other = &renderer.roots[i];
                break;
            }
        if (!pending_other) status = STATUS_QUOTA_EXCEEDED;
        else
        {
            pending_other->root_identity = 0x1122334488776655ull;
            pending_other->root_generation = 13;
            pending_other->input_authorized = TRUE;
            renderer.input_head = renderer.input_count = 0;
            for (i = 0; i < ARRAY_SIZE(renderer.input_events); ++i)
                queue_renderer_input(&other_root, &queued, FALSE);
            if (store_renderer_keyboard_enter(pending_root, 37, &keys) ||
                store_renderer_keyboard_enter(pending_other, 38, &keys))
                status = STATUS_DATA_ERROR;
            for (i = 0; i < ARRAY_SIZE(renderer.input_events) + 16 &&
                    (pending_root->keyboard_snapshot_pending ||
                     pending_other->keyboard_snapshot_pending); ++i)
                if (get_renderer_input(&dequeued))
                {
                    status = STATUS_DATA_ERROR;
                    break;
                }
            if (pending_root->keyboard_snapshot_pending || pending_other->keyboard_snapshot_pending)
                status = STATUS_DATA_ERROR;
            memset(pending_other, 0, sizeof(*pending_other));
        }
    ...
    /* A focus enter which preceded admission is delivered before the held
     * key snapshot. A full queue retains it instead of dropping activation. */
    memset(renderer.input_events, 0, sizeof(renderer.input_events));
    renderer.input_head = renderer.input_count = 0;
    root.keyboard_modifiers_valid = FALSE;
    root.input_authorized = FALSE;
    root.keyboard_focused = root.keyboard_focus_pending = TRUE;
    keys.size = keys.alloc = sizeof(key_data);
    store_renderer_keyboard_enter(&root, 31, &keys);
    if (renderer.input_count) status = STATUS_DATA_ERROR;
    set_renderer_input_authorized(&root, TRUE);
    if (root.keyboard_focus_pending || root.keyboard_snapshot_pending ||
        renderer.input_count != ARRAY_SIZE(expected_codes) + 2 ||
        renderer.input_events[0].type != WINEWAYLAND_HOST_INPUT_FOCUS ||
        !renderer.input_events[0].state ||
        renderer.input_events[1].type != WINEWAYLAND_HOST_INPUT_RESET)
        status = STATUS_DATA_ERROR;
    renderer.input_count = ARRAY_SIZE(renderer.input_events);
    root.keyboard_focus_pending = TRUE;
    queue_renderer_keyboard_focus(&root);
    if (!root.keyboard_focus_pending) status = STATUS_DATA_ERROR;
    renderer.input_count = 0;
    queue_renderer_keyboard_focus(&root);
    leave_renderer_keyboard_root(&root);
    if (root.keyboard_focused || root.keyboard_focus_pending ||
        renderer.input_count != 3 ||
        renderer.input_events[2].type != WINEWAYLAND_HOST_INPUT_FOCUS ||
        renderer.input_events[2].state)
        status = STATUS_DATA_ERROR;
    ...
    free(saved_events);
    if (!status)
        fprintf(stderr, "Vulkan transport self-test: keyboard and pointer state reconciliation passed.\n");
    return status;
}
...
static NTSTATUS cancel_renderer_present(struct renderer_root *root)
{
    ...
    if (root->present_wsi_unavailable) return STATUS_INVALID_DEVICE_STATE;
...
static void retry_renderer_present_retirement(struct renderer_device_queue *queue,
        ...
        NTSTATUS status, VkResult present_result, VkResult failure, BOOL commit_proven)
{
    ...
    retry.u.present.commit_proven = commit_proven;
        ...
        const struct renderer_queue_job *job)
{
    ...
    if (job->type == RENDERER_QUEUE_JOB_PRESENT)
    {
        ...
        commit_proven = TRUE; /* No native Present has been called yet. */
        submit_info.waitSemaphoreCount = 1;
        ...
        if ((vr = renderer.p_vkQueueSubmit(queue->queue, 1, &submit_info, VK_NULL_HANDLE)))
        {
            ...
            retry_renderer_present_retirement(queue, job, RENDERER_QUEUE_JOB_PRESENT_CANCEL,
                    status, VK_NOT_READY, vr, commit_proven);
            return;
        }
        else
        {
    ...
    else if (job->type == RENDERER_QUEUE_JOB_PRESENT_FENCE)
    {
        ...
        commit_proven = job->u.present.commit_proven;
        goto fence;
    }
    else
        present_result = job->u.present.present_result;
    ...
    if (!status && (present_result == VK_SUCCESS || present_result == VK_SUBOPTIMAL_KHR))
    {
        ...
        if (vr == VK_TIMEOUT || vr == VK_ERROR_OUT_OF_HOST_MEMORY || vr == VK_ERROR_OUT_OF_DEVICE_MEMORY)
        {
            if (vr != VK_TIMEOUT) usleep(16000);
...
fence:
        ...
        if (vr)
        {
            if (vr != VK_ERROR_DEVICE_LOST)
            {
                retry_renderer_present_retirement(queue, job, RENDERER_QUEUE_JOB_PRESENT_FENCE,
                        status, present_result, vr, commit_proven);
                return;
            }
    ...
    job->u.present.root->present_commit_proven = commit_proven;
...
static NTSTATUS poll_renderer_root_present(struct renderer_root *root)
{
    ...
    /* A GPU fence retires source reads, not deferred native WSI commits. Keep
     * the present semaphore, surface and geometry token if that boundary was
     * not observed. Only this root becomes unavailable; recovery needs an
     * independently proven native teardown, not another empty queue fence. */
    if (root->present_commit_proven)
    {
    ...
    {
        root->present_wsi_unavailable = TRUE;
...
static NTSTATUS import_renderer_slot(void *args)
{
    ...
    if (params->root_identity)
    {
        ...
        if (root->present_wsi_unavailable)
        {
...
static NTSTATUS process_renderer_frame(void *args)
{
    ...
    if (!(slot->root && slot->root->present_wsi_unavailable) &&
        !renderer_frame_storage_available(queue, slot->root, slot->frame_credit_limit))
        return STATUS_PENDING;
...
static NTSTATUS test_renderer_input(void *args)
{
    ...
    if (!(root = find_renderer_root(params->root_identity, params->root_generation)))
        return STATUS_NOT_FOUND;
    if (renderer.input_count > ARRAY_SIZE(renderer.input_events) -
            7 - root->keyboard_modifiers_valid)
        return STATUS_INSUFFICIENT_RESOURCES;
    keys.size = keys.alloc = sizeof(key_data);
    keys.data = &key_data;
    renderer_keyboard_enter(NULL, NULL, 0, root->surface, &keys);
    renderer_keyboard_key(NULL, NULL, 0, 0, KEY_A, WL_KEYBOARD_KEY_STATE_RELEASED);
    return STATUS_SUCCESS;
}
...
static NTSTATUS present_renderer_root(void *args)
{
    ...
    root->present_commit_proven = FALSE;
...
static NTSTATUS validate_renderer_root_action(struct renderer_root *root,
        const struct winewayland_host_renderer_action *params)
{
    if (root->present_wsi_unavailable || !root->input_authorized || renderer.pointer_root != root ||
...
#define MEMORY_TEST_CHECK(condition) do { if (!(condition)) \
    ...
    root->present_wsi_unavailable = TRUE;
    process.reusable = 0;
    MEMORY_TEST_CHECK(process_renderer_frame(&process) == STATUS_INVALID_DEVICE_STATE && process.reusable);
    MEMORY_TEST_CHECK(memory_test.allocations == 4 && queue_test.signals == 3);
done:
    for (i = 0; i < ARRAY_SIZE(fds); ++i)
...
#define QUEUE_TEST_CHECK(condition) do { if (!(condition)) \
    ...
    for (mode = 0; mode < 8; ++mode)
    {
        ...
        queue_test.wait_result = mode == 5 ? VK_ERROR_OUT_OF_HOST_MEMORY :
                mode == 6 ? VK_ERROR_OUT_OF_DATE_KHR : mode == 7 ? VK_TIMEOUT : VK_SUCCESS;
        ...
        root->configured = TRUE;
        root->swapchain = (VkSwapchainKHR)6;
        ...
        job.u.present.swapchain = root->swapchain;
        ...
        if (mode == 7) queue->count = ARRAY_SIZE(queue->jobs);
        execute_renderer_queue_job(queue, &job);
        if (mode == 1 || mode == 3 || mode == 4 || mode == 5)
        {
            ...
            queue->head = (queue->head + 1) % ARRAY_SIZE(queue->jobs);
            --queue->count;
            QUEUE_TEST_CHECK(retry.type == (mode == 5 ? RENDERER_QUEUE_JOB_PRESENT_WAIT :
                    mode == 4 ? RENDERER_QUEUE_JOB_PRESENT_FENCE :
                    RENDERER_QUEUE_JOB_PRESENT_CANCEL));
            queue_test.wait_result = VK_SUCCESS;
        ...
        if (mode >= 6)
        {
            /* The GPU fence may succeed while no native commit boundary was
             * observed. Complete source reads, but never authorize a newer
             * configure, new WSI job, unmap or native-owner handoff. */
            QUEUE_TEST_CHECK(!root->present_commit_proven && queue_test.presents == 1);
            queue_test.fence_ready = TRUE;
            QUEUE_TEST_CHECK(!poll_renderer_root_present(root));
            QUEUE_TEST_CHECK(root->present_wsi_unavailable && root->geometry_token_revision &&
                    root->present_complete && root->present_semaphore && !root->present_fence);
            QUEUE_TEST_CHECK(root->present_job_status == (mode == 6 ? STATUS_UNSUCCESSFUL : STATUS_DEVICE_BUSY));
            QUEUE_TEST_CHECK(queue_test.fences == 1 && queue_test.semaphores == 1 && queue_test.buffers == 1);
            QUEUE_TEST_CHECK(!release_renderer_frame(&release));
            QUEUE_TEST_CHECK(create_renderer_root_swapchain(root, 64, 64) == STATUS_INVALID_DEVICE_STATE);
            QUEUE_TEST_CHECK(hide_renderer_root(&cancel) == STATUS_INVALID_DEVICE_STATE);
            QUEUE_TEST_CHECK(retire_renderer_root(&cancel) == STATUS_INVALID_DEVICE_STATE);
            QUEUE_TEST_CHECK(cancel_renderer_root(&cancel) == STATUS_INVALID_DEVICE_STATE);
            QUEUE_TEST_CHECK(!poll_renderer_root_present(root));
            QUEUE_TEST_CHECK(queue_test.fences == 1 && queue_test.semaphores == 1 && queue_test.buffers == 1);
            goto iteration_done;
        }
        QUEUE_TEST_CHECK(root->present_commit_proven && !root->present_wsi_unavailable);
        QUEUE_TEST_CHECK(queue_test.submits == (!mode ? 1 : mode == 4 ? 3 : 2));
        QUEUE_TEST_CHECK(queue_test.commands == (mode >= 2));
        QUEUE_TEST_CHECK(queue_test.presents == (mode == 2 || mode == 4 || mode == 5) &&
                queue_test.waits == queue_test.presents + (mode == 5));
